...
class TBCalendarHeaderView: UIView {
    ...
    func setMonthDate(date: NSDate){
        ...
        let monthStr: String = self.month(date: date) + "月" + "\n"
    ...
    func createAttrStr(monthStr: String, yearStr: String) -> NSMutableAttributedString{
        let monthDict = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 24), NSAttributedString.Key.foregroundColor:UIColor.UIColorFromRGB(rgb: 0x4F505F)]
        let yearDict = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 14), NSAttributedString.Key.foregroundColor:UIColor.UIColorFromRGB(rgb: 0x999999)]
